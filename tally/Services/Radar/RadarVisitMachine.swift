...
nonisolated public struct RadarVisit: Identifiable, Hashable, Sendable, Codable {
    ...
    /// The venue's name, cached at entry. Optional because a visit persisted by
    /// a build that predates this field has none.
    ...
    /// is what makes the exit worth a session true-up (SPEC §2).
    ///
    /// The mid-session reminder no longer lives here: it runs off the session
    /// itself (`SessionReminderScheduler`), so it reaches every session rather
    /// than only those inside a Tier 1 geofence.
...
nonisolated public struct RadarVisitMachine: Sendable {
    ...
    nonisolated public struct Configuration: Hashable, Sendable {
        ...
        /// The delay mirrors `TallyDefaults.Fallback.barRadarDwellMinutes`
        /// (45 min), written out because that enum is main-actor isolated and
        /// this machine has to stay usable from anywhere.
        public init(
    ...
    public func handle(_ input: RadarVisitInput, state: RadarVisitState) -> Outcome {
        ...
        case .declined(let visitID, let at):
            ...
            let effects = [cancelDwell(&visit, at: at)].compactMap { $0 }
            state.upsert(visit)
