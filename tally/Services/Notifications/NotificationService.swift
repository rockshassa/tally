...
public final class NotificationService: NSObject {
    ...
    private func refreshWeeklyDigest(
        ...
        if let suppression {
            center.cancel(identifiers: [identifier])
            record(suppression, category: category, identifier: identifier, title: text.title, body: text.body, now: now)
            return
        }

        guard let delivery = deliveryDate(for: fireDate, category: category) else {
            center.cancel(identifiers: [identifier])
            record(.quietHours(until: nil), category: category, identifier: identifier, title: text.title, body: text.body, now: now)
            return
        }
    ...
    private func refreshTrendAlert(
        ...
        let text = NotificationCopy.Trend.text(finding)
        ...
        if let last = TallyDefaults.date(forKey: TallyDefaults.Keys.lastTrendAlertAt),
           now.timeIntervalSince(last) < Self.trendCooldown {
            record(.cooldown, category: category, identifier: identifier, title: text.title, body: text.body, now: now)
            return
        }
        ...
        let content = makeContent(category: category, text: text)
    ...
    func evaluatePacingNudge(
        ...
        if TallyDefaults.string(forKey: TallyDefaults.Keys.lastPacingNudgeSessionID) == session.id.uuidString {
            record(.onePerVisit, category: category, identifier: identifier, title: text.title, body: text.body, now: now)
            return
        }
