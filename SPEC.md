...
- **Mid-session reminder:** once a session is running, undercounting becomes the failure mode — so if no drink has been logged for 60 min (configurable), one quiet nudge: *"Still at The Anchor — anything to add?"* (*"Session in progress — anything to add?"* when the session has no venue) with a **+1 drink** action. It runs off the session itself, not a Bar Radar visit, so every session gets it — tagged or not, Radar on or off. Any log (app, widget, watch, or the notification) resets the timer; **one per quiet stretch** — silence after the reminder earns no second one, and the session closes itself 3 h after its last drink. "Not drinking tonight" silences it for the rest of the session. A session closed early by a geofence exit gets none. Tapping it opens the counter.
...
- **Retention:** on the Tally screen the card is **always there** while recovery context is on: the active episode, else the most recent one however long ago it ended, until a new episode replaces it. The widget keeps a shorter rule — a completed episode stays until the later of **24 h after its last drink** and **24 h after its modeled return**, then the widget drops it. Any episode remains openable from session detail's **Recovery timeline** row, which opens the same complete episode with that session's own drinks highlighted.
- **Y axis:** every app chart of the curve (card, expanded chart, Trends) carries a real vertical scale — round-number gridlines down the leading edge, labelled in display units under the "Modeled suppression above baseline" caption. The widget's curve has no room for one.
- **Trends curve:** Trends shows the modeled curve on every timeframe (Day / Week / Month — the last 14 days, 12 weeks, or 12 months), on the same span as the drinks chart above it. It is sampled finely and thinned by keeping each stretch's maximum, so every night's peak is drawn at its true height; elapsed time is solid and the modeled future dashed.
...
- **Notifications:** the §5 per-category toggles; mid-session reminder interval (default 60 min, §2); quiet-hours window; live permission status.
